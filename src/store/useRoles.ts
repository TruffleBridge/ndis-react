...
export const useRoles = create<RoleStore>((set, get) => ({
    ...
    setRoleName: (roleName) =>
        set((state) => ({ form: { ...state.form, roleName } })),

    setStatus: (status) =>
        set((state) => ({ form: { ...state.form, status } })),
    ...
    setPermissions: (permissions) =>
        set((state) => ({ form: { ...state.form, permissions } })),

    resetForm: () => set({ form: emptyForm, prefillPermissions: null }),
    ...
    deleteRole: async (id) => {
        try {
            ...
            const res = await deleteApiRequest("/roles", { data: { id } });
            if (res.data.status) {
